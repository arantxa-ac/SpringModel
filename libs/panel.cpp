...
bool loadMassSpringModel = false;
bool loadChainPendulumModel = false;
bool loadClothModel = false;
bool loadCubeModel = false;
...
void updateMenu() {
  ...
  if (showPanel && Begin("panel", &showPanel, ImGuiWindowFlags_MenuBar)) {
    ...
    if (CollapsingHeader("Models")) {
      loadMassSpringModel = Button("Mass on a Spring");
      loadChainPendulumModel = Button("Chain Pendulum");
      loadClothModel = Button("Hanging Cloth");
      loadCubeModel = Button("Jelly Cube");
    }
