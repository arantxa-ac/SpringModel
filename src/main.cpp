...
int main(void) {
  ...
  auto defaultModel = std::make_unique<MassOnASpringModel>();
  ...
  mainloop(std::move(window), [&](float) {
    ...
    if (panel::loadMassSpringModel) {
        auto newModel = std::make_unique < MassOnASpringModel>();
        modelRenderable = makeModelRenderable(*newModel, view);
        model = std::move(newModel);
        panel::playModel = false;
    }

    if (panel::loadChainPendulumModel) {
        auto newModel = std::make_unique<ChainPendulumModel>();
        modelRenderable = makeModelRenderable(*newModel, view);
        model = std::move(newModel);
        panel::playModel = false;
    }

    if (panel::loadClothModel) {
        auto newModel = std::make_unique<ClothModel>();
        modelRenderable = makeModelRenderable(*newModel, view);
        model = std::move(newModel);
        panel::playModel = false;
    }
    if (panel::loadCubeModel) {
        auto newModel = std::make_unique<CubeModel>();
        modelRenderable = makeModelRenderable(*newModel, view);
        model = std::move(newModel);
        panel::playModel = false;
    }
    ...
    while (accumulator >= panel::dt) {
        if (panel::playModel || panel::stepModel) {
            model->step(panel::dt);
        }
        accumulator -= panel::dt;
    }
